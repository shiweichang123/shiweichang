					...
					<!-- form表单 -->
						<form @submit.prevent="submit">
							<TextField
								...
								:error="errors.handle"
								info="此处的handle是在后端接口中需要用来查询数据的,通常是写你的Email的名字"
							/>
							<SelectList
								name="status"
								:error="errors.status"
...
<script>
import TextField from './common/TextFieldGroup.vue';
import TextArea from './common/TextAreaGroup.vue';
import SelectList from './common/SelectListGoup.vue';
...
export default {
	...
	methods: {
		getCurrentProfile() {
			...
			// 	profile.skills = '';
			// }
			// let ss = profile.skills.length
			// profile.skills = ss > 0 ? profile.skills.join(',') : '';
			console.log(profile.skills.length)
			this.msgInfo = profile
			console.log(this.msgInfo)
		},
		submit() {
			// console.log(this.msgInfo);
			this.$axios.post("/api/profile",this.msgInfo)
			.then((res) => {
				this.errors = {};
